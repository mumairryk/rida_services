...
class UsersController extends Controller
{
    ...
    public function my_pets(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = MyPets::select('id','name','breed_id','sex','dob','weight','food','additional_notes','active','species as species_id','image','medicine_instructions')->with(['breed'=>function($q){
            ...
            }])->where(['user_id'=>$user_id,'deleted'=>0])->orderBy('id','desc')->get();
            if ($data->count() > 0) {
