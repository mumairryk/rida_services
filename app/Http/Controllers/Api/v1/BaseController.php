...
class BaseController extends Controller
{
    ...
    public function doctors(Request $request)
    {
        ...
        foreach ($data as $key => $row) {
            ...
            foreach ($row->doctor_dates as $key => $val) {
                ...
                $slots[] = $this->getTimeSlots($startTime, $endTime, $interval,$booked_time_slot_array,true,$date);
    ...
    public function getTimeSlots_old($startTime, $endTime, $interval,$booked_time_slot_array=[],$range=true)
    {
    ...
    public function getTimeSlots($startTime, $endTime, $interval,$booked_time_slot_array=[],$range=true,$date='')
    {
        $slots_first_time = [];
        $slots_endslot_time = [];
        foreach ($booked_time_slot_array as $key => $slo) {
            $first = explode(' - ', $slo)[0]??'';
            if($first && !in_array($first, $slots_first_time)){
                $slots_first_time[] = $first;
            }
            $endslot = explode(' - ', $slo)[1]??'';
            if($endslot && !in_array($endslot, $slots_endslot_time)){
                $slots_endslot_time[] = $endslot;
            }
        }
        $start = Carbon::parse($startTime);
        $end = Carbon::parse($endTime);
        ...
        while ($start->lt($end)) {
            ...
            $start_time_only = $start->format('h:i A');
            $end_time = date('h:i A', strtotime(' +'.$interval.' minutes', strtotime($start)));
            if($range){
            ...
            if($interval == 30){
                // $end_time = date('h:i A', strtotime(' +'.$interval + $interval.' minutes', strtotime($start)));
            }
            //note: varify if time is in between the avaible time and slots and per the slots
            if(!in_array($start_time, $slots_first_time) && !in_array($end_time, $slots_endslot_time)){
                $date_time = date('Y-m-d H:i',strtotime($date.$start_time_only));
                
                $date_n = $date ? $date :  time_to_uae(now(),'Y-m-d');
                if($date_time > $date_n){
                    $slots[] = $time_slot;
                }
            }
    ...
    public function getTimeSlots_for_v_and_g($startTime, $endTime, $interval,$booked_time_slot_array=[],$range=true,$doctor_id=0,$date,$now)
    {
        $slots_first_time = [];
        $slots_endslot_time = [];
        foreach ($booked_time_slot_array as $key => $slo) {
            $doc_id = explode('_', $slo)[1]??'';
            $first = explode(' - ', $slo)[0]??'';
            if($first && !in_array($first, $slots_first_time)){
                $slots_first_time[] = $doc_id ? $first.'_'.$doc_id : $first;
            }
            $endslot = explode(' - ', $slo)[1]??'';
            if($endslot && !in_array($endslot, $slots_endslot_time)){
                $slots_endslot_time[] = $endslot;
        ...
        // dd($slots_first_time,$slots_endslot_time);
        $start = Carbon::parse($startTime);
        $end = Carbon::parse($endTime);
        $slots = [];
        while ($start->lt($end)) {
            ...
            $start_time = $doctor_id ? $start_time.'_'.$doctor_id : $start_time;
            $end_time = $doctor_id ? $end_time.'_'.$doctor_id : $end_time;
            //note: varify if time is in between the avaible time and slots and per the slots
            if(!in_array($start_time, $slots_first_time) && !in_array($end_time, $slots_endslot_time)){
                $date_ti = date('Y-m-d H:i',strtotime($date.$start_time_only));
                if($date_ti > time_to_uae(now(),'Y-m-d H:i')){
                    $slots[] = $time_slot;
                }
            }
            $start->addMinutes($interval);
    ...
    public function doctor_timeslots(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $list = DoctorCalender::select('event_date as date', 'start_time', 'end_time','doctor_id')
            ...
            ->groupBy('event_date', 'start_time', 'end_time','doctor_id')->orderBy('event_date', 'asc')->orderBy('start_time', 'asc')->get();


            $vendorIds = VendorModel::where(['role'=>'3','deleted'=>'0'])->orderBy('id','desc')->pluck('id')->toArray();
            
            $holiday_dates = VendorHolidayDates::where([['date','>=',time_to_uae(now(),'Y-m-d')]])->whereIn('vendor_id',$vendorIds)->pluck('date')->toArray();
            $booked_time_slot_arr = [];
            ...
            foreach ($list as $key => $val) {
                ...
                $booked_time_slot_array = ServiceQuotes::
                when($date,function($q) use ($date){
                    return $q->whereDate('date', $date);
                })->where('service_id',1)
                ->whereIn('status',[0,1,3])
                ->where('time_slot','!=','')->orderBy('date','asc')->get()->toArray();
                    
                    $new_booked_time_slot_array = [];
                    foreach ($booked_time_slot_array as $key => $value) {
                        $new_booked_time_slot_array[] = $value['time_slot'].'_'.$value['doctor_id'];
                    }
                ...
                // dd($list);
                if($request->test){
                //     $booked_time_slot_arra1 = ServiceQuotes::
                ...
                //     $new_booked_time_slot_array = [];
                //     foreach ($booked_time_slot_arra1 as $key => $value) {
                //         $new_booked_time_slot_array[] = $value['time_slot'].'_'.$value['doctor_id'];
                //     }
                // dd($new_booked_time_slot_array);
                
                    $booked_time_slot_arr[][$date] = $new_booked_time_slot_array;
                }
                $slot = $this->getTimeSlots_for_v_and_g($startTime, $endTime, $interval,$new_booked_time_slot_array,true,$val->doctor_id,$d_and_time,$now);
                if(count($slot)){
                    $slots[] = $slot;
                    if(!in_array($date, $dates)){
                        array_push($dates, $date);
            ...
            foreach ($slots as $key => $slot) {
                if(!in_array($slot, $new_slots)){
                    array_push($new_slots, $slot);
                }
            }
            usort($new_slots, function($a, $b) {
                $ad = new \DateTime(explode(' - ', $a)[0]);
                $bd = new \DateTime(explode(' - ', $b)[0]);
            ...
            // $slots = $slots;
            if(count($booked_time_slot_arr)){
                $o_data['booked_time_slot_arr'] = $booked_time_slot_arr;
            }
            $o_data['interval'] = $interval;
            $o_data['dates'] = $dates;
            $o_data['slots'] = $new_slots;
    ...
    public function groomer_timeslots(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $list = GroomerCalender::select('event_date as date', 'start_time', 'end_time','groomer_id')
            ...
            ->where('event_date', '>=',time_to_uae(now(),'Y-m-d H:i:s'))
            // ->where('event_date', gmdate('Y-m-d', strtotime($request->date)))
            ->groupBy('event_date', 'start_time', 'end_time','groomer_id')->orderBy('event_date', 'asc')->orderBy('start_time', 'asc')->get();
            ...
            foreach ($list as $key => $val) {
                ...
                $new_booked_time_slot_array = [];
                foreach ($booked_time_slot_array as $key => $value) {
                    $new_booked_time_slot_array[] = $value['time_slot'].'_'.$value['groomer_id'];
                }
                ...
                $slot = $this->getTimeSlots_for_v_and_g($startTime, $endTime, $interval,$new_booked_time_slot_array,true,$val->groomer_id,$d_and_time,$now);
                if(count($slot)){
                    $slots[] = $slot;
                    // dd($slot,$booked_time_slot_array);
                    if(!in_array($date, $dates)){
                        array_push($dates, $date);
                    }
                }
    ...
    public function vendor_availability(Request $request)
    {
        ...
        $where['deleted'] = 0;
        $where['active'] = 1;
        $interval = 60;//$request->interval ?? 60;
        // if(!$request->interval){
        //     if($request->pet_ids){
        //         $pets = MyPets::select('id', 'name', 'breed_id', 'sex', 'dob', 'weight', 'food', 'additional_notes', 'active', 'species as species_id', 'image', 'medicine_instructions')->with(['breed' => function ($q) {
        //             $q->select('breeds.id', 'breeds.name', 'room_type_id as cage_type_id', 'room_types.name as cage_type', 'appoint_time_id', 'appointment_times.name as appointment_time', 'minutes_required as appointment_time_in_minutes')->leftjoin('room_types', 'room_types.id', 'room_type_id')->leftjoin('appointment_times', 'appointment_times.id', 'appoint_time_id');
            ...
            if($boarding_availablity){
                ...
                if($startTime && !in_array($r_date, $holiday_dates)){
                    if($request->time_slot){
                        $startTime = date('h:i A', strtotime(' +'.$interval.' minutes', strtotime($request->time_slot)));
                    }
                    $slots[] = $this->getTimeSlots($startTime, $endTime, $interval,[],$range,$r_date);
                }else{
                   // $holiday_dates[] =  $r_date;
                }
            }
            $slots = call_user_func_array('array_merge', $slots);
        }
