...
class CartController extends Controller
{
    ...
    public function payment_init(Request $request)
    {
        ...
        $messages = [
            ...
            'address_id.required' => trans('validation.address_id_required'),
        ];
        $validator = Validator::make($request->all(),$rules,$messages);
        ...
        if ($validator->fails()) {
            ...
            $user_id = $this->validateAccesToken($request->access_token);
            $cart_details = $this->process_cart($user_id);
            ...
            if ((int) $amount_to_pay == 0) {
                ...
                $temp_order->shipping_charge = $shipping_charge;
                if ($coupon && $discount) {
                ...
                if ($request->payment_type == 2 || $request->payment_type == 3 || $request->payment_type == 4) {
