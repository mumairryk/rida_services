...
class SendPostNottification extends Command
{
    ...
    public function handle()
    {
       ...
       $post =Post::with('post_users','user')->where(['id'=>$post_id,'is_bad_word_exist'=>0])->get();
       if($post->count() > 0){
           sleep(10);
         $post = $post->first();
         $followers_ids = [];
         ...
         if($followers->count() > 0){
           foreach($followers as $followUser){
             //printr($followUser);
             if($followUser->follower->id != $post->user->id){
         ...
         $nodeData = [
          ...
          //  'description'    => $post->user->name." added new post ",
           ...
           'post_image_url'=>(string)$post->file
         ];
         $title = "New post";
         $description = $post->user->name." added new post ";
         ...
         foreach($tag_user_lists as $key){
           if($key->firebase_user_key != ''){
             $nodeData['title'] = " tagged you in a post";
            //  $nodeData['description'] = $post->user->name." tagged you in a post";
             $nodeData['description'] = "tagged you in a post";
             $nottifcation_items[$key->firebase_user_key.'/'.$nottification_id] = $nodeData;
           }
           if($key->user_device_token != ''){
