...
@section('content')
    <div class="card mb-5">
        <div class="card-body">
            ...
            <form method="post" id="admin-form" action="{{ url('admin/manufacturer/update/' .$make->id ) }}" enctype="multipart/form-data" data-parsley-validate="true">
                    <input type="hidden" name="id" value="">
                    @csrf()
                    <div class="row">
                        <div class="col-md-6 mb-2">
                            <div class="form-group">
                                <label>Manufacturer Name<b class="text-danger">*</b></label>
                                <input type="text" name="name" class="form-control" required
                                    data-parsley-required-message="Enter Country Name" value="{{$make->name}}"> 
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                            <div class="form-group">
                                ...
                                <select name="active" class="form-control">
                                    <option {{$make->status == '1' ? 'selected' : ''}} value="1">Active</option>
                                    <option {{$make->status == '0' ? 'selected' : ''}}  value="0">Inactive</option>
                                </select>
                            </div>
                        </div>
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res['status'] == 0) {
                        ...
                        setTimeout(function() {
                            window.location.href = App.siteUrl('/admin/manufacturer');
                        }, 1500);
                    }
