...
<div class="mb-5">
    ...
    <form method="post" id="admin-form" action="{{ url('admin/delivery_representative') }}"
        ...
        <div class="">
            <div class="card mb-2">
                <div class="card-body">
                    <h5 class="card-title mb-4">Fill Your Basic Info</h5>

                    <div class="row">
                        <div class="col-sm-4 col-xs-12">
                            <div class="form-group">
                                <label>Driver Name <span style="color:red;">*<span></span></span></label>
                        ...
                        <div class="col-sm-4 col-xs-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="dob form-control flatpickr-input"
                                    data-date-format="dd-mm-yyyy" name="dob"
                                    value="{{empty($datamain->dob) ? '': date('d-m-Y', strtotime($datamain->dob))}}"
                                    required data-max-date='today' data-parsley-required-message="Enter Date of Birth">
                            </div>
                        </div>
                                ...
                                <div class="input-group">
                                    <div class="input-group-prepend w-100">
                                        <select class="form-control" name="country_id" required
                                            ...
                                            @foreach ($countries as $cnt)
                                            <option @if($id) @if($datamain->country_id==$cnt->id) selected @endif @endif
                                                value="{{ $cnt->id }}">{{ $cnt->name }}</option>
                                            @endforeach;
                                        </select>
                                    </div>
                                </div>
                                <span id="nation_err"></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card mb-2">
                <div class="card-body">
                    <h5 class="card-title mb-4">Bank Account Details</h5>
            ...
            <div class="card p-2 mb-2">
                <h5 class="card-title pt-3 pl-3 mb-4">Fill Your Basic Info</h5>
                <div class="row m-0">
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group d-flex align-items-center">
                            <div>
                                <label> Front Picture of the Car (gif,jpg,png,jpeg) <span
                                        style="color:red;">*<span></span></span></label>
                                <input type="file" class="form-control jqv-input" name="vehicle_front"
                                    data-role="file-image" data-preview="vehicle_front-preview" value="" @if(empty($id))
                                    required data-parsley-required-message="image is required" @endif
                                    data-parsley-trigger="change">
                            </div>
                            <img id="vehicle_front-preview" class="img-thumbnail w-50"
                                style="margin-left: 5px; height:75px; width:75px !important;"
                                src="{{empty($userVehicle->vehicle_front) ? asset('admin-assets/assets/img/placeholder.jpg'): asset($userVehicle->vehicle_front)}}">
                        </div>
                    </div>
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group d-flex align-items-center">
                            <div>
                                <label> Car Picture from Back (gif,jpg,png,jpeg) <span
                                        style="color:red;">*<span></span></span></label>
                                <input type="file" class="form-control jqv-input" name="vehicle_back"
                                    data-role="file-image" data-preview="vehicle_back-preview" value="" @if(empty($id))
                                    required data-parsley-required-message="image is required" @endif
                                    data-parsley-trigger="change">
                            </div>
                            <img id="vehicle_back-preview" class="img-thumbnail w-50"
                                style="margin-left: 5px; height:75px; width:75px !important;"
                                src="{{empty($userVehicle->vehicle_back) ? asset('admin-assets/assets/img/placeholder.jpg'): asset($userVehicle->vehicle_back)}}">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group d-flex align-items-center">
                            <div>
                                <label> Vehicle Registration (gif,jpg,png,jpeg) <span
                                        style="color:red;">*<span></span></span></label>
                                <input type="file" class="form-control jqv-input" name="vehicle_registration"
                                    data-role="file-image" data-preview="vehicle_registration-preview" value=""
                                    @if(empty($id)) required data-parsley-required-message="image is required" @endif
                                    data-parsley-trigger="change">
                            </div>
                            <img id="vehicle_registration-preview" class="img-thumbnail w-50"
                                style="margin-left: 5px; height:75px; width:75px !important;"
                                src="{{empty($userVehicle->vehicle_registration) ? asset('admin-assets/assets/img/placeholder.jpg'): asset($userVehicle->vehicle_registration)}}">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group d-flex align-items-center">
                            <div>
                                <label> Driving License (gif,jpg,png,jpeg) <span
                                        style="color:red;">*<span></span></span></label>
                                <input type="file" class="form-control jqv-input" name="driving_license"
                                    data-role="file-image" data-preview="driving_license-preview" value=""
                                    @if(empty($id)) required data-parsley-required-message="image is required" @endif
                                    data-parsley-trigger="change">
                            </div>
                            <img id="driving_license-preview" class="img-thumbnail w-50"
                                style="margin-left: 5px; height:75px; width:75px !important;"
                                src="{{empty($userVehicle->driving_license) ? asset('admin-assets/assets/img/placeholder.jpg'): asset($userVehicle->driving_license)}}">
                        </div>
                    </div>
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group">
                            <label>Vehicle Type</label>
                            <select name="vehicle_type" class="form-control status-selection">
                                <option @if(!empty($userVehicle)) {{$userVehicle->vehicle_type==1 ? "selected" : null}}
                                    @endif value="1">MotorCycle</option>
                                <option @if(!empty($userVehicle)) {{$userVehicle->vehicle_type==2 ? "selected" : null}}
                                    @endif value="2">Pickup</option>
                                <option @if(!empty($userVehicle)) {{$userVehicle->vehicle_type==3 ? "selected" : null}}
                                    @endif value="3">Normal</option>
                                <option @if(!empty($userVehicle)) {{$userVehicle->vehicle_type==4 ? "selected" : null}}
                                    @endif value="4">Big Truck</option>
                                <option @if(!empty($userVehicle)) {{$userVehicle->vehicle_type==5 ? "selected" : null}}
                                    @endif value="5">Medium Truck</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res['status'] == 0) {
                ...
                setTimeout(function() {
                    window.location.href = App.siteUrl('/admin/delivery_representative');
                }, 1500);
            }
